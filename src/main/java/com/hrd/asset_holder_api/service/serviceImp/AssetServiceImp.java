                ...
                for (JsonNode row : rows) {
                    ...
                    if (!id.equals("\u0000􏿿initialized")) {
                        ...
                        for (JsonNode historyEntry : historyResponse) {
                            System.out.println("hitory -->" + historyEntry);
                            if ((!historyEntry.has("asset_id") || historyEntry.get("asset_id").asText().isEmpty())
                            ) {
                                continue;
                            }

                            String username = historyEntry.has("assign_to") ? historyEntry.get("assign_to").asText() : "unknown";
                            if (username.isEmpty()) {
                                username = "unknown";
                            }
                            ...
                            try {
                                assignToUserId = Integer.parseInt(username);
                            } catch (NumberFormatException e) {
                                System.out.println("Invalid user ID format: " + username);
                                continue;
                            }
                            ...
                            if (isAdmin || assignToUserId == currentUser.getUserId()) {
                                ...
                                if (assignedUser != null) {
                                    ...
                                    userJson.put("fullName", assignedUser.getFullName() != null ? assignedUser.getFullName() : "N/A");
                                    userJson.put("profileImg", assignedUser.getProfileImg() != null ? assignedUser.getProfileImg() : "");

                                    userJson.put("department", assignedUser.getDepartment() != null
                                            ? assignedUser.getDepartment().getDep_name()
                                            : "");

                                    historyWithUserInfo.set("assignTo", userJson);
                                } else {
                                    System.out.println("User not found for ID: " + assignToUserId);
                                }
